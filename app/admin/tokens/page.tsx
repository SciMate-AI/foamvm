    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,_#020617_0%,_#111827_100%)] px-4 py-12 text-slate-100">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-6 rounded-[28px] border border-white/10 bg-white/[0.03] p-6">
          <div className="text-xs uppercase tracking-[0.28em] text-amber-200/70">邀请码管理 / Latest tokens</div>
          <div className="mt-5 overflow-hidden rounded-3xl border border-white/10">
            <table className="min-w-full divide-y divide-white/10 text-left text-sm">
              <thead className="bg-black/20 text-slate-400">
                <tr>
                  <th className="px-4 py-3 font-medium">状态 / Status</th>
                  <th className="px-4 py-3 font-medium">指定邮箱 / Assigned email</th>
                  <th className="px-4 py-3 font-medium">兑换用户 / Redeemed by</th>
                  <th className="px-4 py-3 font-medium">备注 / Note</th>
                  <th className="px-4 py-3 font-medium">创建时间 / Created</th>
                  <th className="px-4 py-3 font-medium">操作 / Action</th>
                </tr>
              </thead>
                  ...
                  <tr key={token.id}>
                    ...
                    <td className="px-4 py-3">
                      {token.status === 'unused' ? (
                        ...
                        <span className="text-xs text-slate-500">已锁定 / Locked</span>
                      )}
                    </td>
                  </tr>
                ))}
